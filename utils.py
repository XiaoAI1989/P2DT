...
import matplotlib
matplotlib.use("Agg")  # headless-safe: training servers usually have no display
...
def get_log_dict(manager=None, num_seeds=0):
    log_keys = ['eval_steps', 'eval_returns', 'd4rl_score', 'action_loss', 'rtg_target']
...
def plot_scores(scores, steps=None, window=100, label=None, color=None):
    ...
    if steps is not None:
        for i in range(len(scores)):
            avg_scores[i] = np.interp(np.arange(steps[i][-1]), [0] + steps[i][-len(avg_scores[i]):],
                                      [0.0] + avg_scores[i])
    if len(scores) > 1:
...
def visualize(step, title, log_dict):
    ...
    if min([len(log_dict['eval_steps'][i]) for i in range(len(log_dict['eval_steps']))]) > 0:
        lines.append(
            plot_scores(log_dict['eval_returns'], log_dict['eval_steps'], window=1, label='eval return', color='C1'))
        plt.ylabel('scores')
        plt.twinx()
